    ...
    def _get_file(self):
        if self.h5py_file is None:
            with self.__lock__:
                if self.h5py_file is None:
                    if self.mode == "r":
                        self.h5py_file = h5py.File(open(self.path, "rb"), self.mode, libver='latest', swmr=True) # use https://docs.h5py.org/en/stable/swmr.html
                    else:
                        self.h5py_file = h5py.File(self.path, self.mode, libver='latest', swmr=True)
